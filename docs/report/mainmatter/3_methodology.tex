...
One of the simplest way to handle this problem is to limit the maximum possible length of the candidate.
...
Thus, the algorithm for candidate extraction wilt bounded maximum length of n-grams are the following.
\begin{algorithm}
    \KwData{\( n \in \mathrm{N} \) -- number of words in the target sentence,
        \(M \leq n \in \mathrm{N} \) -- maximum length of a target candidate}
...
An alternative approach to candidates extraction is to use some model to generate candidates.
