...
\begin{lemma} \label{lemma:swap_source_reduction}
    ...
    \end{align*}
    is also feasible and has the same objective value.
\end{lemma}
\begin{proof}
    ...
    And since by construction sets \( T^*_x \) and \( T^*_{x^*} \) are equal by the corollary \ref{col:maxis_and_ilp_objective_equal}
    the objective values for these solutions are also equal.
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
         ...
         & \forall \src{p} \in S \; \forall \tgt{p} \in T_{add} \qquad \hat{x}_{\src{p}, \tgt{p}} = 0
    \end{align*}
    This solution has the same objective value since all score that for dummy target nodes are equal to \( 0 \)
    by construction and also it \( \hat{x} \) doesn't violate the non-overlapping constraints:
    \begin{align*}
         & \forall (\src{p_1}, \src{p_2}, \tgt{p_1}, \tgt{p_2}) \in \hat{\Pi}(S, T) \Big| \tgt{p_2} \in T_{V} \quad
        \hat{x}_{\src{p_1}, \tgt{p_1}} + \hat{x}_{\src{p_2}, \tgt{p_2}} =
        x_{\src{p_1}, \tgt{p_1}} + x_{\src{p_2}, \tgt{p_2}} \leq 1                                                    \\
         & \forall (\src{p_1}, \src{p_2}, \tgt{p_1}, \tgt{p_2}) \in \hat{\Pi}(S, T) \Big| \tgt{p_2} \in T_{add} \quad
        \hat{x}_{\src{p_1}, \tgt{p_1}} + 0 \leq
        x_{\src{p_1}, \tgt{p_1}} + x_{\src{p_2}, \tgt{p_2}} \leq 1
    \end{align*}

    Because of overlapping constraints \eqref{eq:non_overlap_const} at most one source node can
    ...
    \begin{align*}
         ...
         & \sum\limits_{\tgt{p} \in T} x^*_{\src{\hat{p}}, \tgt{p}} = 1 + \sum\limits_{\tgt{p} \in T} \hat{x}_{\src{p}, \tgt{p}} \\
    \end{align*}
    If this solution doesn't satisfy the desired property repeat the procedure.
\end{proof}
