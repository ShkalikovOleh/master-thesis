...
\begin{theorem}
    ...
    of \( < \) or \( \leq \), such that it projects source entity to a target candidate where any word's index is inside the range \( [m, M] \).
\end{theorem}
\begin{proof}
    ...
    and \( x_{\src{p}, \tgt{\hat{p}}} = 1 \).

    This case can be divided into two parts: \( \tgt{\hat{p}} \) is completely outside of words with indices
    between \( m \) and \( M \) or it overlaps with this range.
