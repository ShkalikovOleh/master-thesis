...
In a general case it makes the initial ILP problem harder, i.e. it can be showed that the problem
...
The generalized number of dummy source nodes for any type of constraints \eqref{eq:num_proj_const} are given
...
In order to overcome this problem, let's create dummy target nodes. The generalized minimum number of such nodes are given
...
candidates from the set \( T \), consider the following set that fully determine the solution of the
MaxIS problem:
\[
...
And finally we can prove the complexity of the generalized ILP problem by reduction the MaxIS problem
