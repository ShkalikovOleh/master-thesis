...
\begin{frame}
  ...
  \begin{figure}
    \begin{tikzpicture}[node distance=0.05,
      ...
      \draw[decoration={brace},decorate] (1.north west) -- node[above=5pt] {$\tgt{p_1}$} (1.north east);
      \draw[decoration={brace},decorate] (2.north west) -- node[above=5pt] {$\tgt{p_2}$} (2.north east);
      \draw[decoration={brace},decorate] (3.north west) -- node[above=5pt] {$\tgt{p_3}$} (3.north east);
      \draw[decoration={brace},decorate] (4.north west) -- node[above=5pt] {$\tgt{p_4}$} (4.north east);
      \draw[decoration={brace},decorate] (5.north west) -- node[above=5pt] {$\tgt{p_5}$} (5.north east);
      \draw[decoration={brace},decorate] (6.north west) -- node[above=5pt] {$\tgt{p_6}$} (6.north east);

      % \node (anchor_p8) at (3.south east)++(0, 1) {};

      \draw[decoration={brace,mirror,raise=5pt},decorate] (1.south west) -- node[below=6pt] {$\tgt{p_7}$} (2.south east);
      \draw[decoration={brace,mirror,raise=5pt},decorate] (2.south west)++(0,-0.7) -- node[below=6pt] {$\tgt{p_8}$} ($(3.south east)+(0,-0.7)$);
      \draw[decoration={brace,mirror,raise=5pt},decorate] (3.south west) -- node[below=6pt] {$\tgt{p_9}$} (4.south east);
      \draw[decoration={brace,mirror,raise=5pt},decorate] (4.south west)++(0,-0.3) -- node[below=6pt] {$\tgt{p_{10}}$} ($(5.south east)+(0,-0.3)$);
      \draw[decoration={brace,mirror,raise=5pt},decorate] (5.south west) -- node[below=6pt] {$\tgt{p_{11}}$} (6.south east);
    \end{tikzpicture}
...
\begin{frame}
  \frametitle{Reduction of the set of target candidates}

  For \( < \) and \( \leq \) type of constraints, the set of candidates can be reduced
  by removing all candidates that lie outside the range between leftmost and rightmost word
  aligned with any entity words.
  ...
  \begin{figure}
    \begin{tikzpicture}[node distance=-0.1,
      ...
      \node[per={B}, rounded rectangle east arc=none](w1_src){$ \src{w_1} $};
      \node[per={I}, rounded rectangle west arc=none, right=of w1_src](w2_src){$ \src{w_2} $};
      \node[right=of w2_src](w3_src){$ \src{w_3} $};
      \node[right=of w3_src](w4_src){$ \src{w_4} $};
      \node[right=of w4_src](w5_src){$ \src{w_5} $};
      \node[loc={B}, right=of w5_src](w6_src){$ \src{w_6} $};

      \node[below=of w1_src, yshift=-1.2cm](w1_tgt){$ \tgt{w_1} $};
      \node[right=of w1_tgt](w2_tgt){$ \tgt{w_2} $};
      \node[right=of w2_tgt](w3_tgt){$ \tgt{w_3} $};
      \node[right=of w3_tgt](w4_tgt){$ \tgt{w_4} $};
      \node[right=of w4_tgt](w5_tgt){$ \tgt{w_5} $};
      \node[right=of w5_tgt](w6_tgt){$ \tgt{w_6} $};
      \node[right=of w6_tgt](w7_tgt){$ \tgt{w_7} $};

      \draw[->] (w1_src) -- (w2_tgt);
      \draw[->] (w2_src) -- (w3_tgt);
      \draw[->] (w2_src) -- (w5_tgt.north);
      \draw[->] (w6_src) -- (w6_tgt);

      \draw[red, thick] (w1_tgt.north east) -- (w1_tgt.south east);
      \draw[red, thick] (w6_tgt.north east) -- (w6_tgt.south east);

      \draw[decoration={brace,mirror,raise=5pt},decorate] (w2_tgt.south west) -- node[below=6pt] {$\tgt{p_1}$} (w3_tgt.south east);
      \draw[decoration={brace,mirror,raise=5pt},decorate] (w7_tgt.south west) -- node[below=6pt] {$\tgt{p_3}$} (w7_tgt.south east);
      \draw[decoration={brace,mirror,raise=5pt},decorate] (w1_tgt.south west)++(0,-0.5) -- node[below=6pt] {$\tgt{p_2}$} ($(w3_tgt.south east)+(0,-0.5)$);
    \end{tikzpicture}
    \caption{All candidates that start or end outside the red lines can be removed}
  \end{figure}

  Note, that \( \forall \src{p} \in S \):
  \[
    c_{\src{p}, \src{p_3}} = 0 \qquad
    c_{\src{p}, \src{p_1}} \geq c_{\src{p}, \src{p_2}}
  \]
\end{frame}
...
\begin{frame}
  ...
  \begin{alertblock}{Crucial difference}
    The heuristic algorithm is capable of merging nearby candidates, whereas
    the ILP-based approach can only do so in specific cases.
  \end{alertblock}
  ...
  \begin{figure}
    \begin{tikzpicture}[node distance=-0.1,
        ...
        every node/.style={text centered,
          text height=1.5ex,
        text depth=.25ex},
        misc/.style={fill=red!30, rounded rectangle,
          % label={[anchor=center,font=\tiny\bfseries\sffamily]above:####1-MISC}
        },
      ]

      \node(w0_src){\dots};
      \node[misc, right=of w0_src, rounded rectangle east arc=none](w1_src){Community};
      \node[misc, right=of w1_src, rounded rectangle west arc=none, rounded rectangle east arc=none](w2_src){competition};
      \node[misc, right=of w2_src, rounded rectangle west arc=none](w3_src){law};
      \node[right=of w3_src](wn_src){\dots};

      \node[below=of w0_src, yshift=-1.2cm] (w0_tgt){\dots};
      \node[right=of w0_tgt](w1_tgt){Wettbewerbsrecht};
      \node[right=of w1_tgt](w2_tgt){der};
      \node[right=of w2_tgt](w3_tgt){Gemeinschaft};
      \node[right=of w3_tgt](wn_tgt){\dots};

      \node[below=of w0_tgt, yshift=-0.2cm](0){\textbf{ILP:}};
      \node[below=of w1_tgt, yshift=-0.2cm](1){B-MISC};
      \node[below=of w2_tgt, yshift=-0.2cm](2){O};
      \node[below=of w3_tgt, yshift=-0.2cm](3){O};

      \node[below=of 0, yshift=-0.2cm] (0){\textbf{Heuristic:}};
      \node[below=of 1, yshift=-0.2cm](1){B-MISC};
      \node[below=of 2, yshift=-0.2cm](1){I-MISC};
      \node[below=of 3, yshift=-0.2cm](1){I-MISC};

      \draw[->] (w1_src) -- (w3_tgt);
      \draw[->] (w2_src) -- (w1_tgt);
      \draw[->] (w3_src) -- (w1_tgt);
    \end{tikzpicture}
    \caption{Example where ILP-based approach fails. Alignments are denoted with edges}
  \end{figure}
\end{frame}
      ...
      \begin{figure}
        \begin{tikzpicture}[node distance=-0.1,
            ...
            every node/.style={text centered,
              ...
              text depth=.25ex,
            font={\scriptsize}},
            org/.style={fill=cyan!30, rounded rectangle,
            },
            misc/.style={fill=red!30, rounded rectangle,
            },
          ]
          ...
          \node(w1_src){Delays};
          \node[right=of w1_src](w2_src){in};
          \node[right=of w2_src](w3_src){the};
          \node[org, right=of w3_src](w4_src){CEN};
          \node[right=of w4_src](w5_src){'s};
          \node[right=of w5_src](w7_src){work};
          \node[right=of w7_src](w8_src){are};
          \node[right=of w8_src](w9_src){now};
          \node[right=of w9_src](w10_src){making};
          \node[below=of w1_src, xshift=-0.3cm](w11_src){it};
          \node[right=of w11_src](w12_src){difficult};
          \node[right=of w12_src](w13_src){to};
          \node[right=of w13_src](w14_src){apply};
          \node[right=of w14_src](w15_src){this};
          \node[right=of w15_src](w16_src){very};
          \node[right=of w16_src](w17_src){directive};
          \node[right=of w17_src](w18_src){.};

          \node[below=of w1_src, xshift=0.37cm, yshift=-1cm](w1_tgt){Aufgetretene};
          \node[right=of w1_tgt](w2_tgt){Verzögerungen};
          \node[right=of w2_tgt](w3_tgt){bei};
          \node[right=of w3_tgt](w4_tgt){den};
          \node[right=of w4_tgt](w5_tgt){Arbeiten};
          \node[below=of w1_tgt, xshift=-0.6cm](w6_tgt){des};
          \node[org, right=of w6_tgt](w7_tgt){CEN};
          \node[right=of w7_tgt](w8_tgt){führen};
          \node[right=of w8_tgt](w9_tgt){nun};
          \node[right=of w9_tgt](w10_tgt){zu};
          \node[right=of w10_tgt](w11_tgt){Problemen};
          \node[right=of w11_tgt](w12_tgt){bei};
          \node[right=of w12_tgt](w13_tgt){der};
          \node[below=of w6_tgt, xshift=0.5cm](w14_tgt){Anwendung};
          \node[right=of w14_tgt](w15_tgt){eben};
          \node[right=of w15_tgt](w16_tgt){dieser};
          \node[misc, right=of w16_tgt](w17_tgt){Richtlinie};

          \node[below=of w1_tgt, yshift=-1.5cm](w1_ilp){Aufgetretene};
          \node[right=of w1_ilp](w2_ilp){Verzögerungen};
          \node[right=of w2_ilp](w3_ilp){bei};
          \node[right=of w3_ilp](w4_ilp){den};
          \node[right=of w4_ilp](w5_ilp){Arbeiten};
          \node[below=of w1_ilp, xshift=-0.6cm](w6_ilp){des};
          \node[org, right=of w6_ilp](w7_ilp){CEN};
          \node[right=of w7_ilp](w8_ilp){führen};
          \node[right=of w8_ilp](w9_ilp){nun};
          \node[right=of w9_ilp](w10_ilp){zu};
          \node[right=of w10_ilp](w11_ilp){Problemen};
          \node[right=of w11_ilp](w12_ilp){bei};
          \node[right=of w12_ilp](w13_ilp){der};
          \node[below=of w6_ilp, xshift=0.5cm](w14_ilp){Anwendung};
          \node[right=of w14_ilp](w15_ilp){eben};
          \node[right=of w15_ilp](w16_ilp){dieser};
          \node[right=of w16_ilp](w17_ilp){Richtlinie};

          \node[text=gray, font=\tiny, above=of w5_src](source){Source labeled sentence};
          \node[text=gray, font=\tiny, below=of source, yshift=-1.1cm](mt){Model transfer};
          \node[text=gray, font=\tiny, below=of mt, yshift=-1.47cm]{ILP with NERScore};
        \end{tikzpicture}
        \caption{Example of ignoring a incorrectly predicted entity}
      \end{figure}
    \end{column}
    \begin{column}[b]{.5\textwidth}
      \begin{figure}
        \begin{tikzpicture}[node distance=-0.1,
          ...
          \node[org](w1_src){Parliament};
          \node[right=of w1_src](w2_src){rejected};
          \node[right=of w2_src](w3_src){the};
          \node[right=of w3_src](w4_src){request};
          \node[right=of w4_src](wn_src){\dots};

          \node[below=of w1_src, xshift=-0.37cm, yshift=-0.7cm] (w0_tgt){Das};
          \node[right=of w0_tgt](w1_tgt){Parlament};
          \node[right=of w1_tgt](w2_tgt){lehnt};
          \node[right=of w2_tgt](w3_tgt){den};
          \node[right=of w3_tgt](w4_tgt){Antrag};
          \node[right=of w4_tgt](wn_tgt){\dots};

          \node[below=of w0_tgt, yshift=-0.7cm] (w0_ilp){Das};
          \node[org, right=of w0_ilp](w1_ilp){Parlament};
          \node[right=of w1_ilp](w2_ilp){lehnt};
          \node[right=of w2_ilp](w3_ilp){den};
          \node[right=of w3_ilp](w4_ilp){Antrag};
          \node[right=of w4_ilp](wn_ilp){\dots};

          \node[text=gray, font=\tiny, above=of w2_src, yshift=0.2cm](source){Source labeled sentence};
          \node[text=gray, font=\tiny, below=of source, yshift=-0.95cm](mt){Model transfer};
          \node[text=gray, font=\tiny, below=of mt, yshift=-0.65cm]{ILP with NERScore};
        \end{tikzpicture}
        \vspace*{1.4cm}
        \caption{Example of restoring the target entity that was missed by the NER model}
      \end{figure}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}[b]{.5\textwidth}
      \begin{figure}
        \begin{tikzpicture}[node distance=-0.1,
          ...
          \node(w1_src){It};
          \node[right=of w1_src](w2_src){is};
          \node[right=of w2_src](w3_src){irresponsible};
          \node[right=of w3_src](w4_src){of};
          \node[org, right=of w4_src, rounded rectangle east arc=none](w5_src){EU};
          \node[org, right=of w5_src,  rounded rectangle east arc=none,  rounded rectangle west arc=none](w6_src){Member};
          \node[org, right=of w6_src,  rounded rectangle west arc=none](w7_src){States};
          \node[below=of w1_src](w8_src){to};
          \node[right=of w8_src](w9_src){refuse};
          \node[right=of w9_src](w10_src){to};
          \node[right=of w10_src](w11_src){renew};
          \node[right=of w11_src](w12_src){the};
          \node[right=of w12_src](w13_src){embargo};

          \node[below=of w1_src, xshift=0.1cm, yshift=-1cm](w1_tgt){Die};
          \node[right=of w1_tgt](w2_tgt){Ablehnung};
          \node[right=of w2_tgt](w3_tgt){einer};
          \node[right=of w3_tgt](w4_tgt){Verlängerung};
          \node[right=of w4_tgt](w5_tgt){des};
          \node[below=of w1_tgt, xshift=0.4cm](w6_tgt){Embargos};
          \node[right=of w6_tgt](w7_tgt){seitens};
          \node[right=of w7_tgt](w8_tgt){der};
          \node[org, right=of w8_tgt](w9_tgt){EU-Mitgliedstaaten};
          \node[right=of w9_tgt](w10_tgt){ist};
          \node[below=of w6_tgt, xshift=0.4cm](w11_tgt){unverantwortlich};

          \node[below=of w1_tgt, xshift=0.1cm, yshift=-1.4cm](w1_ilp){Die};
          \node[right=of w1_ilp](w2_ilp){Ablehnung};
          \node[right=of w2_ilp](w3_ilp){einer};
          \node[right=of w3_ilp](w4_ilp){Verlängerung};
          \node[right=of w4_ilp](w5_ilp){des};
          \node[below=of w1_ilp, xshift=0.4cm](w6_ilp){Embargos};
          \node[right=of w6_ilp](w7_ilp){seitens};
          \node[right=of w7_ilp](w8_ilp){der};
          \node[right=of w8_ilp](w9_ilp){EU-Mitgliedstaaten};
          \node[right=of w9_ilp](w10_ilp){ist};
          \node[below=of w6_ilp, xshift=0.4cm]{unverantwortlich};

          \node[text=gray, font=\tiny, above=of w5_src](source){Source labeled sentence};
          \node[text=gray, font=\tiny, below=of source, yshift=-1.1cm](mt){ILP with AlignScore};
          \node[text=gray, font=\tiny, below=of mt, yshift=-1.4cm]{ILP with NERScore};
        \end{tikzpicture}
        \caption{Example where the alignment-based score outperforms NER model-based}
      \end{figure}
    \end{column}
    \begin{column}[b]{.5\textwidth}
      \begin{figure}
        \begin{tikzpicture}[node distance=-0.1,
            ...
            every node/.style={text centered,
              ...
              text depth=.25ex,
            font={\scriptsize}},
            per/.style={fill=green!30, rounded rectangle,
              % label={[anchor=center,font=\tiny\bfseries\sffamily]above:####1-MISC}
            },
          ]

          \node(w1_src){We};
          \node[right=of w1_src](w2_src){shall};
          \node[right=of w2_src](w3_src){now};
          \node[right=of w3_src](w4_src){hear};
          \node[right=of w4_src](w5_src){Mr};
          \node[per, right=of w5_src](w6_src){Wurtz};
          \node[below=of w1_src, xshift=0.4cm](w7_src){speaking};
          \node[right=of w7_src](w8_src){against};
          \node[right=of w8_src](w9_src){this};
          \node[right=of w9_src](w10_src){request};

          \node[below=of w1_src, xshift=0.1cm, yshift=-1cm](w1_tgt){Wir};
          \node[right=of w1_tgt](w2_tgt){kommen};
          \node[right=of w2_tgt](w3_tgt){nun};
          \node[right=of w3_tgt](w4_tgt){zu};
          \node[per, right=of w4_tgt, rounded rectangle east arc=none](w5_tgt){Herrn};
          \node[per, right=of w5_tgt, rounded rectangle west arc=none](w6_tgt){Wurtz};
          \node[right=of w6_tgt](w7_tgt){,};
          \node[right=of w7_tgt](w8_tgt){der};
          \node[below=of w1_tgt, xshift=0.1cm](w9_tgt){gegen};
          \node[right=of w9_tgt](w10_tgt){den};
          \node[right=of w10_tgt](w11_tgt){Antrag};
          \node[right=of w11_tgt](w12_tgt){spricht};

          \node[below=of w1_tgt, yshift=-1.4cm](w1_ilp){Wir};
          \node[right=of w1_ilp](w2_ilp){kommen};
          \node[right=of w2_ilp](w3_ilp){nun};
          \node[right=of w3_ilp](w4_ilp){zu};
          \node[right=of w4_ilp](w5_ilp){Herrn};
          \node[per, right=of w5_ilp](w6_ilp){Wurtz};
          \node[right=of w6_ilp](w7_ilp){,};
          \node[right=of w7_ilp](w8_ilp){der};
          \node[below=of w1_ilp, xshift=0.1cm](w9_ilp){gegen};
          \node[right=of w9_ilp](w10_ilp){den};
          \node[right=of w10_ilp](w11_ilp){Antrag};
          \node[right=of w11_ilp]{spricht};

          \node[text=gray, font=\tiny, above=of w4_src](source){Source labeled sentence};
          \node[text=gray, font=\tiny, below=of source, yshift=-1.1cm](mt){ILP with AlignScore};
          \node[text=gray, font=\tiny, below=of mt, yshift=-1.4cm]{ILP with NERScore};
        \end{tikzpicture}
        \vspace*{0.45cm}
        \caption{Example where the NER model-based score outperforms alignment-based}
      \end{figure}
    \end{column}
  \end{columns}
\end{frame}
